...
class CrawlerPipeline(object):
    def process_item(self, item, spider):
        ...
        main_target_domain=get_url_belong_main_target_domain(current_url)
        pang_table_name=
        sub_table_name=
        url_table_name=get_http_domain_from_url(current_url).split("/")[-1].replace(".","_")+"_urls"
        ...
        primary_key=
        primary_value=
        write_string_to_sql(str(code),DB_NAME,url_table_name,'code',primary_key,primary_value)
        return item
        """
